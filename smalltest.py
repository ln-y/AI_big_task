...
if __name__ == '__main__':
    ...
    data_module = CustomDataModule(batch_size=batch_size, num_workers=4, test_path='smalltest/test')
    ...
    data_module.set_test_path(os.path.join(os.getcwd(), 'smalltest/fgsm'))
    trainer.test(model, data_module)

    # BIM攻击
    print("Testing on BIM perturbed data")
    data_module.set_test_path(os.path.join(os.getcwd(), 'smalltest/bim'))
    trainer.test(model, data_module)

    # PGD攻击
    print("Testing on PGD perturbed data")
    data_module.set_test_path(os.path.join(os.getcwd(), 'smalltest/pgd'))
    trainer.test(model, data_module)

    # C&W攻击
    print("Testing on C&W perturbed data")
    data_module.set_test_path(os.path.join(os.getcwd(), 'smalltest/c_w'))
    trainer.test(model, data_module)
